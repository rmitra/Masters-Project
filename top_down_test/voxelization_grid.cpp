...
void find_min_max(pcl::PointCloud<pcl::PointXYZ> cloud, pcl::PointXYZ &min, pcl::PointXYZ &max){
...
void voxelize(grid *g, pcl::PointCloud<pcl::PointXYZ> cloud, pcl::PointCloud<pcl::Normal> cloud_normals, pcl::KdTreeFLANN<pcl::PointXYZ> &kdtree_grid, int threshold){
	
	g->allocate_points_to_grid(cloud, cloud_normals);
	g->remove_voxels(threshold);	
