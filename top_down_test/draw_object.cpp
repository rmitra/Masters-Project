...
void draw_grid(){
	...
	glBegin(GL_QUADS);
		...
		for(int j = 0; j<2; j++)
		  {
			glVertex3f( g.faces[j].at(0).at(0)*scalex, g.faces[j].at(0).at(1)*scaley, g.faces[j].at(0).at(2)*scalez );
	        glVertex3f( g.faces[j].at(1).at(0)*scalex, g.faces[j].at(1).at(1)*scaley, g.faces[j].at(1).at(2)*scalez );
	        glVertex3f( g.faces[j].at(2).at(0)*scalex, g.faces[j].at(2).at(1)*scaley, g.faces[j].at(2).at(2)*scalez );
	        glVertex3f( g.faces[j].at(3).at(0)*scalex, g.faces[j].at(3).at(1)*scaley, g.faces[j].at(3).at(2)*scalez );
	      }		
	...
	glBegin(GL_QUADS);               
		  ...
		  if(step_wise){
			...
			for(int i = 0; i <= box_no; i++){
				if(!show_cloud)
					glColor4f(c_list[i].r, c_list[i].g, c_list[i].b, alpha);
				else
...
void initialise_parameters(double resolution, GLdouble scale_i, vector<Block> &block_list, int length, int width, int height, pcl::PointXYZ min, pcl::PointXYZ max){
	...
	set_cuboid_c(g, 0, 0, 0, grid_length, grid_width, grid_height);
	
	list_of_blocks = block_list;
	
	n = box_no = block_list.size();
	
	//cout<<"Inside init() n = "<<n<<"\n";
